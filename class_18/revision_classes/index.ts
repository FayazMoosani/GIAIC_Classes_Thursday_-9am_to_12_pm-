...
let count: number = 10; // A block-scoped variable 'count' of type number initialized to 10
if (true) {
  let count: number = 20; // A different block-scoped variable 'count' inside this block, initialized to 20
...
const pi: number = 3.14; // A block-scoped constant 'pi' of type number initialized to 3.14
// pi = 3.15; // Error: Cannot assign to 'pi' because it is a constant

// 3. Using `var` for function-scoped variables
var name: string = "Alice"; // A function-scoped variable 'name' of type string initialized to "Alice"
if (true) {
  var name: string = "Bob"; // The same function-scoped variable 'name' reinitialized to "Bob"
  console.log(name); // Outputs: Bob
}
console.log(name); // Outputs: Bob (var is function-scoped, not block-scoped)

// 4. Primitive types
let age: number = 25; // A variable 'age' of type number initialized to 25
let greeting: string = "Hello"; // A variable 'greeting' of type string initialized to "Hello"
let isActive: boolean = true; // A variable 'isActive' of type boolean initialized to true
let data: null = null; // A variable 'data' of type null initialized to null
let notAssigned: undefined = undefined; // A variable 'notAssigned' of type undefined initialized to undefined
let sym: symbol = Symbol("unique"); // A variable 'sym' of type symbol initialized to a unique symbol
let bigNumber: bigint = 100n; // A variable 'bigNumber' of type bigint initialized to 100n

// 5. Object types
let person: { name: string; age: number } = { name: "Alice", age: 25 }; // An object with properties 'name' and 'age'
let numbers: number[] = [1, 2, 3]; // An array of numbers
let tuple: [string, number] = ["Alice", 25]; // A tuple with a string and a number
...
// 7. Special types
let randomValue: any = 42; // A variable of type 'any' that can hold any value
randomValue = "Hello"; // Reassigned to a string
randomValue = true; // Reassigned to a boolean

let unknownValue: unknown = 42; // A variable of type 'unknown' that can hold any value, but requires type checking before use
if (typeof unknownValue === "number") {
  let value: number = unknownValue; // Type assertion required to use 'unknownValue' as a number
...
// Define an interface MyObject with properties name, class, and an optional age
interface MyObject {
  name: string;
  class: string;
  age?: number; // Make age optional since it's not initially present
}
